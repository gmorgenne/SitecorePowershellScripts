...
$pages = @( Get-Item -Path $pagePath ) + @( Get-ChildItem $pagePath -recurse) 
#$pages = @( Get-ChildItem $productPagesPath -recurse) #+ @( Get-ChildItem $projectTemplates -recurse) + @( Get-ChildItem $featureTemplates -recurse)
...
foreach ($page in $pages) {
	...
	#	Write-Host $page.Name " missing page view component"
	#	Add-Rendering -Item $page -Instance $renderingInstance -Datasource $datasourceId -Placeholder $placeholder
	#}
	if ($renderingToDetermineUpdate.Length -gt 1) {
		Write-Host $page.Name " has multiple page view components"
		Remove-Rendering -Item $page -Instance $renderingToDetermineUpdate[-1]
	}
}
