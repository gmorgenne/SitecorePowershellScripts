...
foreach ($glove in $GlovesProducts) {
	...
	if ($variantOptionsRendering.Length + $variantFilterQuickViewRendering.Length -gt 1) {
		Write-Host $glove.Name " has more than 1 rendering"
		Remove-Rendering -Item $glove -Instance $variantOptionsRendering -Device $QuickViewDevice
	}
}
